...
def plot_actions_over_time(timesteps_list, observations_list, actions_list, reward_func, state, scenario, num_faces,
    ...
    # save_dir = f'../logs/{reward_func}/{state}/{scenario}/BestAgent/{num_faces}faces/{seed}seed/'
    save_dir = f'../logs/{scenario}/{reward_func}/{state}/BestAgent/{num_faces}faces/{seed}seed/'
    os.makedirs(save_dir, exist_ok=True)

    # Save the plot
    plt.savefig(f'{save_dir}BestAgent-{num_faces}-{seed}-Actions.pdf', format='pdf', dpi=1200, bbox_inches='tight',
                transparent=True)

    # plt.savefig(f'{save_dir}BestAgent-{num_faces}-{seed}-Actions.svg', format='svg', dpi=1200, bbox_inches='tight',
    #             transparent=True)
    # plt.show()
...
def BestAgent(reward_func, state, scenario, face):
    # Logger configuration used both inside and outside NDNRouterEnv
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    os.makedirs(f'../logs/{scenario}/{reward_func}/{state}/BestAgent', exist_ok=True)
    file_name = f'../logs/{scenario}/{reward_func}/{state}/BestAgent/Best_Agent_{current_time}.log'
    # Configure a file handler for logging
    file_handler = logging.FileHandler(file_name)
    file_handler.setLevel(logging.INFO)

    # Create a logger instance
    logger: Logger = logging.getLogger('NDN_logger')
    ...
    for seed in tqdm(seeds):
        ...
        log_dir = f'../logs/{scenario}/{reward_func}/{state}/BestAgent/{num_faces}faces/{seed}seed'
        ...
        logger.info(f"\nEvaluating the Best Agent [{num_faces}-{seed}]:")
        ...
        for current_step in range(total_timesteps):
            ...
            obs0 = obs[0]
            env.unwrapped.scenario_dynamic()      # should be disabled in the ndn_router step function
            action = env.unwrapped.get_best_face()
            actions_list.append(action)
            obs, reward, done, _, _ = env.step(action)

            # if reward <= -1:
            #    print(f"step : {current_step} | prefix :'{obs0} | action : {action} | reward : {reward}")

            total_rewards += reward
            # logger.info(f"observation is: {obs}")

        total_rewards_per_seed.append(total_rewards)
        print(f"Total reward of the Best agent [Faces {num_faces} - Seed {seed}]: {total_rewards}")
        logger.info(f"Total reward of the Best agent [{num_faces}-{seed}]: {total_rewards}")

        draw_interval = 1
        plot_actions_over_time(timesteps_list, observations_list, actions_list, reward_func, state, scenario,
                               num_faces, seed,
                               draw_interval)
...
def main():
    ...
    parser.add_argument('--reward_func', type=str, help='reward_func to use')
    ...
    BestAgent(args.reward_func, args.state, args.scenario, args.face)

    # rewards = ["simple", "streak"]
    # states = ["state_one", "state_two", "state_three"]
    # scenarios = ["streak_disruption", "static_with_one_optimal_face", "Dynamic_RTT", "permanent_face_specific",
    #              "transient_face_specific", "permanent_faulty_face", "transient_faulty_face"]
    #
    # BestAgent(rewards[0], states[1], scenarios[0])
