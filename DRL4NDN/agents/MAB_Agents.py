...
from MultiArmedBandit import (EGreedyMultiArmedBandit, UCBMultiArmedBandit,
                              GaussianThompsonSampling, SoftmaxMultiArmedBandit)
...
mab_agents = [EGreedyMultiArmedBandit,
              UCBMultiArmedBandit,
              GaussianThompsonSampling,
              SoftmaxMultiArmedBandit]
...
def agents(reward_func, state, scenario, agent, face):
    ...
    os.makedirs(f'../logs/{scenario}/{reward_func}/{state}/MAB Agents/{agent.__name__}/', exist_ok=True)
    file_name = f'../logs/{scenario}/{reward_func}/{state}/MAB Agents/{agent.__name__}/{agent.__name__}_{current_time}.log'
    ...
    print(f"Evaluating the {mab.__class__.__name__} Agent [Faces {num_faces}]")
    ...
    for seed in tqdm(seeds):
        ...
        log_dir = f'../logs/{scenario}/{reward_func}/{state}/MAB Agents/{agent.__name__}/{num_faces}faces/{seed}seed'
        os.makedirs(log_dir, exist_ok=True)

        logger.info(f"\nEvaluating the {mab.__class__.__name__} Agent [{num_faces}-{seed}]:")
        ...
        for current_step in range(100):
            action_mab = mab.select_arm()
            obs, reward, done, _, _ = train_env.step(action_mab)
            mab.update_arm(action_mab, reward)  # Update MAB based on the observed reward
        ...
        for current_step in range(total_timesteps):
            ...
            action_mab = mab.select_arm()
            actions_list.append(action_mab)
            obs, reward, done, _, _ = eval_env.step(action_mab)
            mab.update_arm(action_mab, reward)  # Update MAB based on the observed reward
            total_rewards += reward

        total_rewards_per_seed.append(total_rewards)
        print(
            f"Total reward of the {mab.__class__.__name__} Agent [{num_faces}-{seed}]: {total_rewards}")
        logger.info(
            f"Total reward of the {mab.__class__.__name__} Agent [{num_faces}-{seed}]: {total_rewards}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--agent', type=str, help='Agent to use (EGreedyMultiArmedBandit, UCBMultiArmedBandit, '
                                                  'GaussianThompsonSampling, SoftmaxMultiArmedBandit)')
    parser.add_argument('--face', type=int, help='face to use')

    args = parser.parse_args()

    agent_classes = {'EGreedyMultiArmedBandit': EGreedyMultiArmedBandit, 'UCBMultiArmedBandit': UCBMultiArmedBandit,
                     'GaussianThompsonSampling': GaussianThompsonSampling,
                     'SoftmaxMultiArmedBandit': SoftmaxMultiArmedBandit}
