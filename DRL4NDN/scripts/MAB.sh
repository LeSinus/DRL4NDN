...
agents=("EGreedyMultiArmedBandit" "UCBMultiArmedBandit" "GaussianThompsonSampling" "SoftmaxMultiArmedBandit")
        ...
        for state in $(echo "$states" | tr ',' ' '); do
            for face in $(echo "$faces" | tr ',' ' '); do
                for agent in "${agents[@]}"; do
                  ...
                  python3 ../agents/MAB_Agents.py --reward "$reward" --state "$state" --scenario "$scenario" --agent "$agent" --face "$face"
                done
            done
        done
    done
